...
## 🟢 현재 프론티어 (2026-07-19 저녁) — AI 자원 접근 2단계: 결핍 주도 채취 전초 (✅ dotnet 빌드 클린 / ⚠ 에디터 실측 대기)
> **유저 선택**: 다음 주제 = 2단계(AI가 광맥·유전을 찾아 광산/시추/항만 자율 배치).
> **AiExtractionPlanSystem(신규, HourChanged 게이트·메인 저빈도)**: AI 팀별(IsPlayer 제외 —
>   AiCityGrowth 규약)·품목별(IronOre·Oil):
>   ① 결핍 = 풀 셀 Target>0 && Stored<Target(인구 앵커 — 소비자 없어도 비축 목표가 견인)
>   ② "가동 중 채취장(발밑 잔량 있음)" 있으면 스킵 — 고갈 시 자동으로 다음 광맥 계획(확장 자연 발생)
>   ③ 노동 게이트(유인 광산만, 무직 ≥ 정원/4 — AiCityGrowth와 동일 임계) ④ ResourceLayer 1회
>   스캔 → 앵커(CityGrid.Anchor) 최근접 유효(비점유·비적영토) 셀 ⑤ 발행:
>   · 광산 = PlaceBuildingRequest(RequireRoadAccess=false) + **RoadPathRequest(RegisterSpur=1)** —
>     2026-06-23 예정됐던 Destination Road 본용도 가동(재니터 자가수리 지선)
>   · 시추 = 배치만(무인·해상) + 항만 없으면 **유전 인근 해안**(링 탐색: 전수 물·비점유·육지
>     접촉≥1·높이 균일, 회전 0/90) 항만 동시 계획 → TankerSystem이 배·운송 자동 연결.
> **키 해석 = 전부 능력 파생(하드코딩 0)**: 채취장 = ProducerLookup.Table(ProductionJob),
>   TypeId·해상 여부 = 프리팹 엔티티의 ResourceExtractor/OffshoreSupplier 컴포넌트 읽기,
>   항만 = **ProducerLookup.PortMainKey 신설**(WarehouseTag.SeaRange>0 최소 키 — 파생 시
>   일반 창고 레인(WarehouseMainKey)에서 항만 제외하는 위생 픽스 동반).
> **창고-먼저 재사용**: 광산 출력이 커버 밖이면 기존 LogisticsMissLog(WarehouseId) 채널이
>   창고를 뒤따라 짓는다 — 이 시스템은 창고를 직접 계획하지 않음(기계 중복 0).
> **검증 포인트**: AI 도시 인구>0 상태에서 1게임시간 내 [ExtractPlan] 로그(P{n} IronOre/Oil
>   채취장·항만 계획) → 광산에 도로 지선 자라남(RoadSpur — 끊으면 재니터 수리) → Miner 통근·
>   생산 → 창고 미스 채널로 인근 창고 → AI 유조선 스폰·왕복 / 광맥 고갈 시 다음 광맥에 신규 광산.
> ⚠ 미결(후속): 고갈 채취장 철거·RoadSpur 해제(발행 주체 소관 규약 — 현재 방치), AI 소비자
>   (제련·정제) 없어 비축 도달 후 유휴 = 정상(MRP 소비 사슬은 다음 주제), 광산 입구 미정의 =
>   footprint 인접 도로 폴백(주택과 동일).

## 🟢 (2026-07-19 실측 7차, 종결 — ✅ 유저 "이제 정상" 확인) 유령 항만 진범 = **학교 프리팹 오편집**
> **유저 결정타**: "학교를 항구로 보고 있다"(F11=Port) → school_h_m.prefab 확인: **IsWarehouse=1 +
>   WarehouseSeaRange=25 + StampMaxDist=30 + ProvidedJob=10(Administrator) + Oil Store 1000** —
>   항만에 넣으려던 값 일습이 학교에 입력돼 있었음(인스펙터에 학교가 열린 채 항만 세팅한 오편집).
>   AI가 교육 수요(오늘 아침 픽스로 활성화)로 학교를 지을 때마다 내륙 유령 항만 탄생 —
>   (45,26) SeaRange=25·"항만 물없음" 연속 경고·AI 유조선 전부 이걸로 설명(5·6차의 스테일 베이크
>   추정은 **오진**이었음 — 물증 기록 정정).
> **조치**: ① 학교 프리팹 원복(ProvidedJob=7 Teacher, Stocks=[], IsWarehouse=0, SeaRange=0)
>   ② **BuildingAuthoring OnValidate 오편집 경보**: 창고+방문서비스 동시 설정 / SeaRange>0인데
>   IsWarehouse 꺼짐 → 저장 시점 경고(같은 사고 재발 시 즉시 발각).
> ⚠ 흥미 부산물: 학교가 Administrator 직장이었으므로 Teacher 근무 0 = 학교 Unstaffed였을 것 —
>   원복으로 교육 서비스도 정상화. 검증: 재실행 후 학교 F11=School·교사 고용, 내륙 항만 경고 소멸.

## 🔧 (2026-07-19 실측 6차) 안전성 예외 픽스 + 내륙 유령 항만 물증 확보
> **유저 로그 2건(구 빌드)**: ① InvalidOperationException — UnitMovementArbiterJob(UnitMoveTarget RW)
>   미완료 상태에서 메인스레드 룩업 접근("처음부터 존재" = 기존 위생 결함, 유조선으로 발현 빈도↑)
>   → **UnitMoveOrderSystem·TankerSystem OnUpdate 서두에 state.CompleteDependency()** (요청 프레임·
>   저빈도 게이트 한정이라 비용 국소). ② "P0 항만 @(45,26) SeaRange=25 물 없음" 연속 —
>   **맵 데이터 검증으로 물증 확보**: (45,26) 주변 12셀 전부 육지·최근접 물 17셀 = 스캔 정상,
>   내륙에 SeaRange=25(항만의 **옛** authored 값 — 현재 프리팹은 5) 창고형 건물이 실존.
>   유저는 항만 배치 안 함 + 세이브 없음(세션 내 생성) → **P0 자율 성장(AI)이 세운 것** =
>   스테일 베이크(옛 SeaRange 25 + 옛 BuildableOn) 잔재와 부합. 경고는 엔티티당 1회로 dedup.
> **유저 체크리스트**: ① Origin.unity 우클릭 **Reimport**(리베이크) ② 재실행 ③ (45,26) 건물이
>   항만 메시인지 확인(F11=Port) ④ 리베이크 후에도 내륙 항만이 새로 서면 재보고(다른 원인 추적).

## 🔧 (2026-07-19 실측 5차) 유령 항만 진단 — AI 일반 창고가 SeaRange>0으로 잡힘 (스테일 베이크 추정)
> **유저 로그(구 빌드)**: AI 소유 "항만 ... 물 없음" 경고 + AI 유조선이 유저 시추를 보고
>   "일감 없음(내소유 0)" → AI 창고(1005)들이 **SeaRange>0 유령 항만**으로 잡히고 있었다는 증거.
>   코드상 AI가 1013을 짓거나 SeaRange를 얻는 경로는 전무(창고 레인 1005 확인·L2/생산 테이블/베이스
>   구성 전수 확인·맵 Singles에 1013 없음 확인) → **유일 잔여 가설 = WarehouseTag에 SeaRange 필드
>   추가 후 서브씬 엔티티 바이너리 미리베이크(스테일 레이아웃) → 쓰레기 값**.
> **조치**: ① "물 없음" 경고에 SeaRange 값 병기 — 비정상 값(수백~수만)이면 스테일 베이크 확정
>   ② (4차) 시추 없는 소유자 스폰 보류 게이트가 증상(AI 유령선·경고 스팸)은 이미 차단
>   ③ **유저 액션 = 서브씬 리베이크**(Origin.unity 우클릭 Reimport 또는 서브씬 열고 저장) 후
>   F11로 AI 창고가 Port 아닌 Warehouse로 표시되는지 확인.

## 🟢 (2026-07-19 실측 4차) "AI 쪽 배" 관찰 — 스폰 게이트 강화 + AI 항만 경로 부재 확인
> **유저 관찰**: "항만 없이도 배가 생기나? AI 쪽에 안 움직이는 배가 보이고, AI가 항만을 짓는
>   시도도 하는 듯" → **코드 확인 결과**: ① 배는 항만(SeaRange>0 창고) 없이는 스폰 경로 자체가
>   없음(TankerSystem 스폰 루프가 유일한 생성점, [Tanker] 스폰 로그로 전수 추적 가능)
>   ② **AI는 현재 항만(1013)을 지을 수 있는 채널이 없음** — 창고 레인 = min-MainKey 창고(1005
>   stock_h=logis_h_l 확인), L2(공급 능력 파생)·생산자 테이블(ProductionJob 파생) 어디에도 1013
>   해석 경로 없음. 팩션 베이스 구성도 HQ(1000)뿐. "AI 항만 시도"로 보인 건 해안가 일반 창고(1005)
>   건설일 가능성 / "AI 쪽 배"는 맵 소품(Megacity boat_001 등 환경 데코) 또는 플레이어 배일 가능성 —
>   콘솔 [Tanker] 스폰 로그의 P번호·좌표로 판별.
> **개선**: 유조선 스폰 게이트에 "소유자 시추 ≥1" 조건 추가 — 석유 산업 없는 항만의 유령선 방지
>   (첫 시추가 서면 다음 게이트에 자동 스폰). AI가 석유 산업(시추+항만+배)을 갖추는 건 2단계
>   (AI 자원 접근 — 결핍 채널→ResourceLayer 스캔→Destination Road) + 해상 확장 설계 과제.

## 🟢 (2026-07-19 실측 3차, 해결) 유조선 정지의 실제 원인 = 항만 SeaRange 반경 밖 시추 → 하드 반경 은퇴
> **유저 실측 확정**: 원인은 시추가 항만 반경(25셀) 밖 — "이 범위가 꼭 필요한가?" → **불필요 판정**:
>   육상 stamp 반경은 이전이 즉시(텔레포트)라 반경이 필수지만, 배는 거리=왕복 시간=처리량 감소라는
>   **연속 자연 비용**을 이미 지불(3단계에선 긴 항로=습격 취약 보급선). 하드 컷오프 = 이중 규제 +
>   조용한 정지 함정.
> **변경**: ① TankerSystem 일감 탐색에서 반경 제거(자기 소유 시추 전체, 최근접 우선)
>   ② SeaRange 의미 축소 = **항만 표식**(>0 = 유조선 스폰) + **텔레포트 폴백 전용 반경**
>   ③ OffshorePush 접속 판정: 유조선 모드 = "항만 하나라도 보유"(miss는 항만 전무일 때만),
>   폴백 모드 = 기존 반경 유지. Authoring 툴팁·컴포넌트 주석 갱신.
> (2차에서 함께 나간 ProductionSystem `Target>0` 부트스트랩 보호·진단 로그는 유지 — 인구 0
>   소유자 채취 가능 + [Tanker]/[UnitPath] 관찰 창구.)

## 🔧 (2026-07-19 실측 2차) 유조선 자동 운항 불가 — 생산 게이트 인구 앵커 구멍 + 진단 로그
> **유저 실측**: "수동 선택 이동은 되는데 자동 이동·운송이 안 됨" → 수동=Direct(무경로)·자동=A*
>   경로라는 차이에서 착안해 2갈래 추적. **유력 근본 원인 발견**: ProductionSystem 비축 게이트에
>   부트스트랩 보호 부재 — 소유자 인구 0이면 Target=0 → `Stored(0)≥Target(0)` 참 → 채취 사이클
>   영영 시작 불가 → 시추 Output 0 → 유조선 Docked에서 일감 없음(정지). AiCityGrowth 쪽엔 있던
>   `Target>0` 가드가 여기만 빠져 있었음 → 추가(무인구 소유자의 채취·생산 부트스트랩 허용).
> **진단 로그 추가(원인 확정용)**: [Tanker] 스폰/일감 스캔(시추 n/내소유/반경내/출하대기)/적재/하역,
>   [UnitPath] naval 경로 실패(출발→목표) — 다음 실행 콘솔이 끊긴 지점을 특정한다.
> **검증**: 콘솔에서 ① [Tanker] 스폰 로그 ② Docked "일감 없음"이면 카운트로 원인 판별
>   (출하대기 0 = 생산 문제 / 반경내 0 = 거리·소유 문제) ③ [UnitPath] 경고 나오면 물 마스크/그리드.

## 🟢 (2026-07-19 실측 픽스) 유조선 정지 버그 — 내비 그리드가 맵의 4%만 커버
> **유저 실측**: "전부 정상인데 유조선만 안 움직임" → 원인 = UnitNavigationGrid 50×50×CellSize2
>   (월드 100×100)인데 맵은 256×256×2(월드 512×512). 그리드 밖 출발/목표 → A* IsInside 실패
>   → PathFailed → **HasTarget=0 = 영구 정지**(TankerSystem이 재발행해도 매번 실패).
>   선택 유닛이 멀쩡해 보인 이유: 직접 명령은 Direct(무경로)라 그리드 무관.
> **픽스 2겹**: ① 씬 그리드 256×256(맵 1:1)·MaxSearchNodes 20000 ② 그리드 밖 출발/목표는
>   경로 시도 없이 **Direct 폴백**(UnitPathfinding.IsInsideGrid 사전 검사 — "시도 후 실패=정지"와
>   "커버 밖=직진"의 구분). + 물 마스크 재빌드 조건에 그리드 크기 변경 추가(리베이크 대응).
> **에디터 검증 부산물(같은 날)**: 광산·시추 프리팹 ProductionOutput=None + Output 재고 칸
>   누락/무효 발견 → 직접 수정(IronOre/Oil, 40). 맵 도색 검증: 철 52셀 전부 육상·유전 41셀 전부
>   수상 ✓. 유조선 TargetType=1(Ground) — 대함 무기 도입 때 Naval(8)로 교체 필요(무해 잔항목).

## 🟢 (2026-07-19 후속) — 해상 이동 도메인 + 유조선 실체 운송 (✅ dotnet 빌드 클린 / ⚠ 에디터 선행 = 유조선 프리팹 + TankerPrefabAuthoring 배치)
