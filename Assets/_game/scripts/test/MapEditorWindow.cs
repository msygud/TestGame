...
namespace CitySim.MapEditor
{
    ...
    public class MapEditorWindow : EditorWindow
    {
        ...
        void DrawPlacementOverlay(SceneView sceneView)
        {
            ...
            {
                // 원점 셀 기준으로 GO 존재 여부 확인
                var originCell = kv.Key;
                // Single인 경우 원점 셀이 별도일 수 있으므로 occupancy 키가 원점
                // _placedObjects는 원점 셀로 저장되어 있음 → 직접 확인
                bool hasGo = _placedObjects.TryGetValue(kv.Key, out var go) && go != null;
                if (hasGo) continue;
