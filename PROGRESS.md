...
## 🟢 현재 프론티어 (2026-07-07 세션 종료) — 시민 경제 + 욕구 주도 배치
> 성능 잡화는 완료(1x 5~7ms / 120x 9~11ms, 인구 2만). 이후 세션 전체가 시민 트랙으로 이동 —
> 아래 "🔴 잡화" 섹션은 이제 대부분 완료·역사. **현재 활성 작업은 파일 하단** "욕구 주도 배치" 및
> 그 위의 시민 트랙 섹션들(고용·교대·재고소비·예약·수요집계) 참조.
>
> **⚠ 다음 세션 최우선 2가지:**
> 1. **미검증 대량 누적** — 이번 세션 시민 코드(고용 1·2차·교대·처리량·예약·서비스데스크·수요집계·
>    인스펙터·수요히트맵) 다수 미검증. **컴파일 + 새 플레이로 정리 먼저.** ⚠ STEP 0(NeedMaps)은
>    **서브씬 재베이크** 필요. F12로 심야 맥동 사라졌는지(STEP 1) 확인.
> 2. **욕구 주도 배치 STEP 2/3 대기** — 설계 확정됨(파일 하단 wf_b416ef08 섹션). STEP 0·1 완료.
>    **유저 결정 대기: 예산분배(권장 A=틱당 서비스1개)** 확정 후 AiCityGrowth 본체 착수.
...
- ⬜ **STEP 2(WHAT-only)·STEP 3(WHERE 바이어스)** — AI 본체. 유저 결정(예산 분배 등) 확인 후 착수.
- ⬜ 다음: STEP 2/3 / 교통 혼잡 / 시민 사망·이탈(불만·행복도 제네릭) / 두 번째 욕구(제네릭 검증).
