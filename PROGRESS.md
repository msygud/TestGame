...
## 🟢 현재 프론티어 (2026-07-10 세션 종료) — 창고 공유 풀 + 배치 시스템 일반화 (드라이버 앵커·인테리어·재개발)
...
>    (창고 20/식당 10)·이민율·재개발 게이트(RedevelopEscalate/Cap=2/2, 거리 32) 튜닝. **관찰 포인트: 도시
>    공동화(재개발 과다) vs 소화 부족.**
> 2. **재건축 쿨다운** — 느린 통계 lag(1초 집계 + HourChanged stamp 재빌드)로 커버 반영 전 같은 수요 재선택
>    → 배치한 `(owner,셀,res)`에 쿨다운(유저 관찰: "지었는데 또 짓는다"). delta 기준선이 대부분 잡지만 lag 잔존.
> 3. **철거 성능** — 재개발 대상 탐색이 `요청수 × 주택수`(DayChanged 저빈도지만 대도시에선 공간 인덱스로).
> 4. **B-2 용량(과부하)** / 하드코딩→"프리팹 접근 구도" / 히트맵 감쇠 / 유닛 롤업(전투 세션).
