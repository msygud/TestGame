...
namespace CitySim.MapEditor
{
    ...
    public class MapEditorWindow : EditorWindow
    {
        ...
        void DrawPlacementOverlay(SceneView sceneView)
        {
            ...
            // ① GO 없는 점유 셀만 보조 오버레이
            //  Single은 Size > 1×1이면 원점 GO를 찾아야 함
            foreach (var kv in _occupancy)
            {
                bool hasGo;
                if (kv.Value == PlacementKind.Single)
                {
                    hasGo = false;
                    foreach (var p in currentMap.Singles)
                    {
                        var pItem = FindItem(p.MainKey, p.VariantKey);
                        var pSize = ValidSize(pItem);
                        if (kv.Key.x >= p.CellX && kv.Key.x < p.CellX + pSize.x &&
                            kv.Key.y >= p.CellZ && kv.Key.y < p.CellZ + pSize.y)
                        {
                            hasGo = _placedObjects.TryGetValue(
                                new Vector2Int(p.CellX, p.CellZ), out var pGo) && pGo != null;
                            break;
                        }
                    }
                }
                ...
                {
                    hasGo = _placedObjects.TryGetValue(kv.Key, out var go) && go != null;
                }
                if (hasGo) continue;
