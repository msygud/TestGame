...
namespace CitySim
{
    ...
    public static class RoadShapeMapping
    {
        ...
        public static bool IsValidMask(byte mask)
            => mask > 0 && mask <= (N | E | S | W);

        public static byte ToMask(RoadDirection directions)
            => (byte)((byte)directions & (N | E | S | W));

        public static byte DefaultMask(RoadShape shape) => shape switch
        {
        ...
        {
            byte basemask = DefaultMask(shape);
