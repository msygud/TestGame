    ...
    public class PrefabRegistryWindow : EditorWindow
    {
        ...
        void DrawItemGroups()
        {
            ...
            for (int i = 0; i < itemsProp.arraySize; i++)
            {
                ...
                if ((RoadShape)shapeProp.enumValueIndex != RoadShape.NotRoad)
                {
            ...
            DrawGroup("Road", roads, reg, ref foldRoads,
                () => {
                    ...
                    item.RoadShape  = RoadShape.Straight;
        ...
        void DrawItem(int index, GamePrefabRegistry reg)
        {
            ...
            bool isRoad  = (RoadShape)roadShapeProp.enumValueIndex != RoadShape.NotRoad;
            ...
            if (isRoad)
            {
                EditorGUILayout.PropertyField(roadShapeProp,
                    new GUIContent("Road Shape"));
            }
            else
            {
                // 비도로: 숨기거나 NotRoad 고정
                EditorGUI.BeginDisabledGroup(true);
                EditorGUILayout.LabelField("Road Shape", "NotRoad", EditorStyles.miniLabel);
                EditorGUI.EndDisabledGroup();
            }
