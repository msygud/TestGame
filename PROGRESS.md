...
- ✅ **대규모 실측 검산(유저, 인구 17,840)** — pursuing 1150 = Travel 519 + Eat 591 + UNMET 40 정확 일치
  ...
  - ⚠ 다음 주시: **ServiceSearchSystem**(메인스레드, 시민 전수 순회 — 대부분 조기 스킵이라 아직 쌀 것)
    — 정체가 남으면 이쪽. stamp 조회가 랜덤 액세스라 잡화 시 설계 필요.
- ⬜ 다음 후보(시민 트랙): 욕구 종류 확장(Hunger 외) / 해소를 재고 소비로 연결(물류 합류 지점 —
  StockEntry 차감) / 직장(WorkplaceBuilding) 태깅 경로 / 시민 사망·이탈(정원 Release 경로) /
  **미충족 욕구 통계 정식화(UNMET → 셀/구역별 집계 = 욕구 주도 배치 입력)**.
