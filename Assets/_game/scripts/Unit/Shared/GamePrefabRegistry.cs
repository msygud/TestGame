...
namespace CitySim
{
    ...
    public class GamePrefabRegistry : ScriptableObject
    {
        ...
        public RegistryItem AddItem()
        {
            var item = new RegistryItem
            {
                ...
                ObjectType = PrefabObjectType.Building,
                Domain     = PrefabDomain.Land,
                PurposeFlags = PrefabPurposeFlags.None,
                RequiredTechLevel = 0,
                SpawnMode  = PrefabSpawnMode.Single,
                Size       = new Vector2Int(1, 1),
                Offset     = Vector3.zero,
                AllowedTerrains = MapTerrainMask.Land,
                RequiresFlatFootprint = true,
                RequiresRoadAdjacency = false,
                DoesNotBlockOccupancy = false,
                RoadShape  = RoadShape.NotRoad,
                RoadDirections = RoadDirection.None,
            };
            items.Add(item);
        ...
        public List<ValidationIssue> Validate()
        {
            ...
            var seenRoadKeys = new Dictionary<(byte directions, int variantKey), int>();

            for (int i = 0; i < items.Count; i++)
            {
                var it = items[i];
                if (it.IsDeleted) continue;
                byte roadMask = it.GetRoadDirectionMask();
                bool roadItem = IsRoadItem(it);
                bool markedAsRoad = IsMarkedAsRoad(it);
                ...
                if (roadItem && roadMask == 0)
                    issues.Add(new ValidationIssue(ValidationLevel.Error, i,
                        $"({it.MainKey},{it.VariantKey}): Road item must define RoadDirections."));

                if (roadMask != 0 && !RoadShapeMapping.IsValidMask(roadMask))
                    issues.Add(new ValidationIssue(ValidationLevel.Error, i,
                        $"({it.MainKey},{it.VariantKey}): RoadDirections mask {roadMask} is invalid."));

                if (roadMask != 0 && !markedAsRoad)
                    issues.Add(new ValidationIssue(ValidationLevel.Warning, i,
                        $"({it.MainKey},{it.VariantKey}): RoadDirections is set but item is not marked as Road."));

                if (roadMask != 0)
                {
                    var roadKey = (roadMask, it.VariantKey);
                    if (seenRoadKeys.TryGetValue(roadKey, out int previousIndex))
                    {
                        issues.Add(new ValidationIssue(ValidationLevel.Error, i,
                            $"Duplicate road key Directions={roadMask}, VariantKey={it.VariantKey}: " +
                            $"item {i} conflicts with item {previousIndex}."));
                    }
                    else
                    {
                        seenRoadKeys[roadKey] = i;
                    }
                }

                // Size 검사 (도로는 1x1 강제)
                if (roadMask != 0 && (it.Size.x != 1 || it.Size.y != 1))
                {
                    issues.Add(new ValidationIssue(ValidationLevel.Warning, i,
            ...
            IEnumerable<GamePrefabRegistry> registries)
        {
            ...
            var seenRoadKeys = new Dictionary<(byte directions, int variantKey), (string regName, int idx)>();
            ...
            foreach (var (it, src, li) in allItems)
            {
                ...
                var roadKey = (roadMask, it.VariantKey);
                if (seenRoadKeys.TryGetValue(roadKey, out var prev))
                {
                    issues.Add(new ValidationIssue(ValidationLevel.Error, li,
                        $"[Merged] Duplicate road key Directions={roadMask}, VariantKey={it.VariantKey}: " +
                        $"'{src.DlcName}'[{li}] conflicts with '{prev.regName}'[{prev.idx}]."));
                }
                else
                {
                    seenRoadKeys[roadKey] = (src.DlcName, li);
                }
            }
        ...
        static bool IsRoadItem(RegistryItem item)
            => item != null
            && (item.ObjectType == PrefabObjectType.Road
                || item.Category == PrefabCategory.Road
                || item.RoadShape != RoadShape.NotRoad
                || item.GetRoadDirectionMask() != 0);

        static bool IsMarkedAsRoad(RegistryItem item)
            => item != null
            && (item.ObjectType == PrefabObjectType.Road
                || item.Category == PrefabCategory.Road
                || item.RoadShape != RoadShape.NotRoad);
    ...
    public class RegistryItem
    {
        ...
        public PrefabObjectType ObjectType = PrefabObjectType.Building;
        public PrefabDomain Domain = PrefabDomain.Land;
        public PrefabPurposeFlags PurposeFlags = PrefabPurposeFlags.None;
        public int RequiredTechLevel;
        [Tooltip("Terrain surfaces this item can be placed on. Structures use this instead of a buildable cell layer.")]
        public MapTerrainMask AllowedTerrains = MapTerrainMask.Land;
        public bool RequiresFlatFootprint = true;
        public bool RequiresRoadAdjacency = false;
        public bool DoesNotBlockOccupancy = false;
        ...
        [Tooltip("Road direction flags. This is the authoritative road prefab key.")]
        public RoadDirection RoadDirections = RoadDirection.None;
        ...
        public byte GetRoadDirectionMask()
        {
            byte mask = RoadShapeMapping.ToMask(RoadDirections);
            return mask != 0 ? mask : RoadShapeMapping.DefaultMask(RoadShape);
        }

        public MapTerrainMask GetAllowedTerrainMask()
        {
            if (ObjectType == PrefabObjectType.Terrain)
                return MapTerrainMask.All;

            return AllowedTerrains != MapTerrainMask.None
                ? AllowedTerrains
                : MapTerrainMask.Land;
        }

        public bool ShouldBlockOccupancy()
            => ObjectType != PrefabObjectType.Terrain && !DoesNotBlockOccupancy;

        public PlacementRuleFlags GetPlacementRuleFlags()
        {
            var rules = PlacementRuleFlags.None;
            if (RequiresFlatFootprint)
                rules |= PlacementRuleFlags.RequiresFlatFootprint;
            if (RequiresRoadAdjacency)
                rules |= PlacementRuleFlags.RequiresRoadAdjacency;
            if (ShouldBlockOccupancy())
                rules |= PlacementRuleFlags.BlocksOccupancy;
            return rules;
        }
    }
    ...
    public enum PrefabObjectType : byte
    {
        Terrain,
        Road,
        Building,
    ...
    [Flags]
    public enum PrefabPurposeFlags : uint
    {
        None        = 0,

        // Citizen stress counters and city services.
        Shelter      = 1u << 0,
        Food         = 1u << 1,
        Rest         = 1u << 2,
        Entertainment = 1u << 3,
        ...
        // Strategic and military intents.
        Command      = 1u << 15,
        Construction = 1u << 16,
        Production   = 1u << 17,
        Scout        = 1u << 18,
