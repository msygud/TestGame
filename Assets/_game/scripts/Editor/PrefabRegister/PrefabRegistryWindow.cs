...
namespace CitySim.MapEditor
{
    ...
    public class PrefabRegistryWindow : EditorWindow
    {
        ...
        // ── 검증 캐시 ─────────────────────────────────────────────
        List<ValidationIssue> cachedIssues = new();
        bool validationDirty = true;   // true면 다음 안전 시점에 재검증
        ...
        void OnGUI()
        {
            ...
            if (serializedReg.ApplyModifiedProperties())
                validationDirty = true;

            // 포커스가 없는 프레임에서만 재검증 — 입력 중 레이아웃 변경 방지
            if (validationDirty && GUI.GetNameOfFocusedControl() == "")
                RefreshValidation();
        }
        ...
        // ══════════════════════════════════════════════════════════
        //  검증 메시지 — 캐시된 결과만 표시 (매 프레임 재검증 없음)
        // ══════════════════════════════════════════════════════════
        void RefreshValidation()
        {
            var reg = registries[selectedRegistryIdx];
            cachedIssues = reg != null ? reg.Validate() : new List<ValidationIssue>();
        ...
        // ══════════════════════════════════════════════════════════
        static string GroupTitle(string label, PrefabCategory cat)
        {
            var (min, max) = MainKeyRange.For(cat);
            string range = max == int.MaxValue ? $"{min}+" : $"{min}~{max}";
            return $"{label}  [M: {range}]";
        }
        ...
        void DrawItemGroups()
        {
            ...
            DrawGroup(GroupTitle("환경 (Environment)", PrefabCategory.Environment), environments, reg, ref foldEnvironment,
                () => { var item = reg.AddItem(); item.Category = PrefabCategory.Environment; });
            EditorGUILayout.Space(5);

            DrawGroup(GroupTitle("전투유닛 (CombatUnit)", PrefabCategory.CombatUnit), combatUnits, reg, ref foldCombatUnit,
                () => { var item = reg.AddItem(); item.Category = PrefabCategory.CombatUnit; });
            EditorGUILayout.Space(5);

            DrawGroup(GroupTitle("투사체 (Projectile)", PrefabCategory.Projectile), projectiles, reg, ref foldProjectile,
