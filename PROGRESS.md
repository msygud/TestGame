...
- ⬜ 다음 후보(시민 트랙): UNMET 셀별 집계(욕구 주도 배치 입력) / 교통 혼잡 / 시민 사망·이탈 /
  거절 쿨다운·숙련 감쇠·직업 선호 배정·분할 근무(밸런스 페이즈).

### 🔎 미완: 시민-시뮬 변경분 적대적 리뷰 (2026-07-07, 백그라운드 워크플로 wf_e5f3161b)
> 이 세션 누적 미검증 코드(고용 1·2차, 예약, 서비스 데스크, 교대)가 상호작용이 커 적대적 리뷰 착수.
> 5개 차원(데드락/좌석누수/스케줄/생산결합/ECS안전) × 발견별 독립 검증. 결과는 다음 턴에 반영.
